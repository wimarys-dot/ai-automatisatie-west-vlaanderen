      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-[#ff6d5a]/10 border border-[#ff6d5a]/20 px-4 py-1.5 rounded-full mb-6">
            ...
            <span className="text-xs font-bold text-[#ff6d5a] tracking-wide">GRATIS AUDIT</span>
          </div>
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-4">
            Ontdek uw <span className="text-[#ff6d5a]">besparingspotentieel</span>
          </h2>
        ...
        <div className="relative">
          <div className="absolute -inset-1 bg-gradient-to-r from-[#ff6d5a] via-[#7c3aed] to-[#0ea5e9] rounded-3xl blur opacity-20"></div>

          <div className="relative bg-[#18181b] border border-[#27272a] rounded-3xl p-8 md:p-10">
            {/* Progress bar */}
            {!result && (
              <div className="mb-8">
                ...
                <div className="h-1 bg-[#27272a] rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-[#ff6d5a] to-[#7c3aed] transition-all duration-500"
            ...
            {step === 1 && !result && (
              <div className="animate-fade-in">
                <h3 className="text-xl font-bold text-white mb-6">In welke sector is uw bedrijf actief?</h3>
                    ...
                    <button
                      ...
                      className={`p-4 rounded-xl border text-left transition-all ${
                        data.sector === sector.value
                          ? 'bg-[#ff6d5a]/10 border-[#ff6d5a] text-white'
                          : 'bg-[#09090b] border-[#27272a] text-zinc-400 hover:border-zinc-600'
                      }`}
                    >
                      {sector.label}
                    </button>
            ...
            {step === 2 && !result && (
              <div className="animate-fade-in">
                <h3 className="text-xl font-bold text-white mb-6">Waar is uw bedrijf gevestigd?</h3>
            ...
            {step === 3 && !result && (
              <div className="animate-fade-in">
                <h3 className="text-xl font-bold text-white mb-6">Hoe groot is uw team?</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {SIZES.map(size => (
                    ...
                    >
                      ...
                      <div className="text-sm text-zinc-500">{size.desc}</div>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {step === 4 && !result && (
              <div className="animate-fade-in">
                <h3 className="text-xl font-bold text-white mb-2">Welke tools gebruikt u dagelijks?</h3>
                <p className="text-zinc-500 text-sm mb-6">Selecteer alle tools die van toepassing zijn</p>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {TOOLS.map(tool => (
                    <button
                      key={tool.value}
                    ...
                    >
                      <span className="flex items-center gap-2">
                        {data.tools.includes(tool.value) && (
                          <svg className="w-4 h-4 text-[#ff6d5a]" fill="currentColor" viewBox="0 0 20 20">
            ...
            {step === 5 && !result && !loading && (
              <div className="animate-fade-in">
                <h3 className="text-xl font-bold text-white mb-6">Wat is uw grootste uitdaging?</h3>
                <div className="space-y-3">
                  {CHALLENGES.map(challenge => (
                    ...
                    >
                      ...
                      <div className="text-sm text-zinc-500">{challenge.desc}</div>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {loading && (
              <div className="py-12 text-center animate-fade-in">
                <div className="w-16 h-16 border-4 border-[#ff6d5a]/20 border-t-[#ff6d5a] rounded-full animate-spin mx-auto mb-6"></div>
                <h3 className="text-xl font-bold text-white mb-2">Uw audit wordt gegenereerd...</h3>
                <p className="text-zinc-500">Onze AI analyseert uw bedrijfsprofiel</p>
              </div>
            )}
            ...
            {!loading && !result && rawResponse && (
              <div className="py-8 animate-fade-in">
                <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-6 mb-6">
                  ...
                  <p className="text-zinc-400 text-sm">{rawResponse}</p>
                </div>
                <button
                  onClick={resetAudit}
                  className="w-full bg-[#27272a] hover:bg-[#3f3f46] text-white px-6 py-4 rounded-xl font-bold transition-all"
                >
                  Opnieuw proberen
                </button>
              </div>
            ...
            {result && (
              <div className="animate-fade-in">
                <div className="flex items-center justify-between mb-8">
                  <div>
                    <h3 className="text-2xl font-bold text-white">Uw Automatisatie Audit</h3>
                    <p className="text-zinc-500">Gepersonaliseerd voor uw bedrijf</p>
                  </div>
                  <div className="text-right">
                    <div className="text-3xl font-bold text-[#ff6d5a]">{result.totalHoursSaved}u</div>
                    <div className="text-xs text-zinc-500 uppercase tracking-wider">per week te besparen</div>
                  </div>
                </div>

                {/* Summary */}
                <div className="bg-[#09090b] border border-[#27272a] rounded-2xl p-6 mb-6">
                ...
                <div className="space-y-4 mb-8">
                  {result.opportunities.map((opp, i) => (
                    <div key={i} className="bg-[#09090b] border border-[#27272a] rounded-2xl p-6">
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-[#ff6d5a]/20 rounded-lg flex items-center justify-center text-[#ff6d5a] font-bold">
                            {i + 1}
                          </div>
                      ...
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-zinc-500">
                          <span className="text-[#ff6d5a] font-bold">{opp.hoursSaved} uur</span> besparing per week
                        </span>
                        <span className="text-zinc-600">{opp.workflow}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {/* Next Step */}
                <div className="bg-gradient-to-r from-[#ff6d5a]/10 to-[#7c3aed]/10 border border-[#ff6d5a]/20 rounded-2xl p-6 mb-6">
                  <div className="flex items-center gap-2 text-[#ff6d5a] text-xs font-bold uppercase tracking-wider mb-2">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" />
                    </svg>
                    Volgende stap
                  </div>
                  <p className="text-white">{result.nextStep}</p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <a
                    href="#contact"
                    className="flex-1 bg-[#ff6d5a] hover:bg-[#ff8575] text-white px-6 py-4 rounded-xl font-bold text-center transition-all"
                  >
                    Gratis gesprek inplannen
                  </a>
                  <button
                    onClick={resetAudit}
                    className="flex-1 bg-[#27272a] hover:bg-[#3f3f46] text-white px-6 py-4 rounded-xl font-bold transition-all"
                  >
                    Nieuwe audit
                  </button>
                </div>
                ...
                {step < totalSteps ? (
                  <button
                    ...
                    className={`px-8 py-3 rounded-xl font-bold transition-all ${
                      canProceed()
                        ? 'bg-[#ff6d5a] hover:bg-[#ff8575] text-white'
